...
if(!"Biaya_Akuisisi_Pelanggan_Juta_IDR" %in% names(data_bersih) || !"Biaya_Akuisisi_Pelanggan_Juta_IDR" %in% names(data_bersih)) {
  stop(paste("Satu atau kedua kolom ('",  "Biaya_Akuisisi_Pelanggan_Juta_IDR", "', '", "Nilai_Pelanggan_Juta_IDR", "') tidak ditemukan. Periksa kembali nama kolom pada file 04_analisis_korelasi.R"))
}
...
scatter_plot <- ggplot(data_bersih, aes_string(x =  "Biaya_Akuisisi_Pelanggan_Juta_IDR", y =  "Nilai_Pelanggan_Juta_IDR")) +
  geom_point(alpha = 0.6, color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") + # Menambah garis tren linear
  labs(
    title = paste("Scatter Plot antara",  "Nilai_Pelanggan_Juta_IDR"
...
correlation_test <- cor.test(data_bersih[["Biaya_Akuisisi_Pelanggan_Juta_IDR"]], data_bersih[["Nilai_Pelanggan_Juta_IDR"]], method = "pearson")

print(paste("--- Hasil Uji Korelasi antara", "Biaya_Akuisisi_Pelanggan_Juta_IDR", "dan", "Nilai_Pelanggan_Juta_IDR", "---"))
...
# Ambil nilai korelasi & p-value
cor_value <- as.numeric(correlation_test$estimate)
p_value <- correlation_test$p.value

cat("--- Interpretasi Sederhana ---\n")
...
cat(
  ...
  "dan",
  strength,
  "antara Biaya_Akuisisi_Pelanggan_Juta_IDR dan Nilai_Pelanggan_Juta_IDR.\n"
)
...
if (p_value <= 0.05) {
  cat(
    ...
    "menunjukkan hasil yang signifikan secara statistik (≤ 0.05),",
    "sehingga terdapat bukti adanya korelasi di populasi.\n"
  )
} else {
  cat(
    "p-value =", round(p_value, 5),
    "tidak signifikan secara statistik (> 0.05),",
    "sehingga korelasi yang teramati dapat terjadi secara kebetulan.\n"
  )
}
